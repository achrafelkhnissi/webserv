/*
    ! problem: The server still varlnable to a type of denial of service attack
    ! solution: 
    !           - use event driven programming (select, epoll, kqueue, etc.)
    !           - Asynchronous I/O (aio_read, aio_write, etc.)
...
void* thread_function(void* arg)
{
    while(true) {
        ...
        if ((pclient_socket = dequeue()) == NULL) {
            pthread_cond_wait(&condition_var, &mutex);

            // try again
            pclient_socket = dequeue();
        }
